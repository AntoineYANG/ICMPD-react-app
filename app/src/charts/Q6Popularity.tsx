 ...
 * @Last Modified time: 2021-02-08 22:12:33
 */
...
for (let y = 1955; y <= 2020; y++) {
  sumPerYear.push(0);
  riverPerYear.push({});
}

const artists = allArtists.filter(a => getGenreName(a.gid) === "Pop/Rock");
...
for (let y = 1955; y <= 2020; y++) {
  // countPerYear.push(0);
  countPerYear.push((1 - proj(sumPerYear[y - 1955])) / 2);
}
...
artists.forEach(d => {
  ...
  for (let j = start; j <= end; j++) {
    ...
    if (h >= proj(sumPerYear[year]) * 0.08 && j >= 1990 && j < 2000) {
...
console.log(Object.keys(isArtistActive).map(d => parseInt(d)));


let popularity = allDev.map(p => {
  return {
    ...p,
    value: p.value.filter(v => v[0] >= 1955)
  };
});
...
popularity = popularity.filter(p => p.genre === "Pop/Rock");
...
const Q6Popularity: React.FC = () => {
  ...
  const fx = (year: number) => LEFTSPAN + (year - 1955) / 70 * WIDTH;
    ...
    <svg width={ WIDTH + LEFTSPAN + RIGHTSPAN } height={ HEIGHT }
      style={{
        ...
        <g key="genre-popularity" >
          ...
          }
          {
            [sumPopularity].map(genre => {
              const path = `M${fx(1955)},${fy1(0)} ` + genre.value.map(d => {
        ...
        <g key="style-change" >
          ...
          {
            poprock.map((d, i) => {
              const min = Math.min(...d.data.filter(p => p[0] >= 1955).map(p => p[1]));
              const max = Math.max(...d.data.filter(p => p[0] >= 1955).map(p => p[1]));
              const style = labelPath(i);
              const path = d.data.filter(p => p[0] >= 1955).map((p, j) => {
            ...
            }} />
          {
            artists.map((d, i) => {
              ...
              for (let j = start; j <= end; j++) {
                data.push([
                  j,
                  riverPerYear[j - 1955][d.aid][0],
                  riverPerYear[j - 1955][d.aid][1]
                ]);
              }
              ...
              return (
                <path key={ d.aid }
                  d={ path }
                  style={{
                    fill: d3.schemeCategory10[i % d3.schemeCategory10.length],
                    stroke: "rgb(174,175,173)",
                    strokeWidth: 0.4,
                    fillOpacity: isArtistActive[d.aid] ? 0.33 : 0.33
                  }}
                  onMouseMove={
                    e => {
                      document.getElementById("focusLine")!.setAttribute("x2", e.pageX - 63 + "");
                  ...
                  onMouseOut={
                    e => {
                      e.currentTarget.style.fillOpacity = "0.33";
                    }
                  } />
              );
            })
